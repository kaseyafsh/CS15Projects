...
value (depending on whether they are leaves and checkInternalValues) and
frequency (depending on checkFrequency) as well as having equal left
and right subtrees.
...
Parameters:
    ...
    checkInternalValues - If set to true, the values of internal nodes (in
addition to leaves) must be equal between the 2 trees.

Returns:
    true if the two trees are equal as defined above and false otherwise.

Example Usage:
    The autograder tests for deserialize_tree in phase 1 use this function with
checkFrequency and checkInternalValues set to false.
*/
